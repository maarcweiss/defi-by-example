...
contract LiquidityExamples2 is IERC721Receiver {
    ...
    INonfungiblePositionManager public nonfungiblePositionManager = 
        ...
        //USDC/USDT POOL
 INonfungiblePositionManager public nonfungiblePositionManager2 = 
        INonfungiblePositionManager(0x3416cF6C708Da44DB2624D63ea0AAef7113527C6);
    ...
    function _createDeposit2(address owner, uint _tokenId) internal {
        (
            ,
            ,
            address token0,
        ...
        ) = nonfungiblePositionManager2.positions(_tokenId);
        // set the owner and data for position
        // operator is msg.sender
        deposits[_tokenId] = Deposit({
            owner: owner,
  ...
  function mintNewPosition2()
        ...
        )
    {
        ...
        // Providing liquidity in both assets means liquidity will be earning fees and is considered in-range.
        uint amount0ToMint = 100 * 1e6;
        uint amount1ToMint = 100 * 1e6;

        // Approve the position manager
        TransferHelper.safeApprove(
            USDT,
            address(nonfungiblePositionManager2),
            amount0ToMint
        );
        TransferHelper.safeApprove(
            USDC,
        ...
        // Note that the pool defined by USDT/USDC and fee tier 0.01% must 
        // already be created and initialized in order to mint
        (_tokenId, liquidity, amount0, amount1) = nonfungiblePositionManager2
            .mint(params);

        // Create a deposit
        _createDeposit2(msg.sender, _tokenId);
